...
bun run test              # or: pytest, cargo test, go test
...
bun run check:file-length # File size limit
bun run test                 # Tests
```
...
For step-by-step guides on common tasks, see **[Development Guide](guides/development.md)**:

- Adding database fields/tables
- Adding API routes (Hono)
- Data fetching patterns (React Router + TanStack Query)
...
- [ ] `bun run precommit` passes (all quality gates)
