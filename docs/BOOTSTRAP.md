...
|------|----------|----------------|
| **Web App** | React, Vue, React Router v7 (+ Hono), Next.js, SvelteKit | Full bootstrap (tests + E2E + lint) |
| **API/Backend** | Express, FastAPI, REST service, Hono | Tests + lint (skip E2E) |
...
|---|------|----------------|--------|
| 1 | **Initialize project** | `bun init -y` / `pip init` / equivalent | Package file exists |
| 2 | **Add TypeScript/typing** | See language-specific section | Type errors caught |
| 3 | **Add linter** | ESLint / Pylint / Clippy / equivalent | `bun run lint` works |
| 4 | **Add formatter** | Prettier / Black / rustfmt | `bun run format` works |
| 5 | **Add test framework** | Vitest / Jest / pytest / equivalent | `bun run test` works |
...
```bash
# Core
bun i -D typescript @types/node
bun i -D vitest
bun i -D eslint @typescript-eslint/parser @typescript-eslint/eslint-plugin
bun i -D prettier eslint-config-prettier
bun i -D husky

# Initialize
bunx tsc --init  # Then enable strict mode
bunx husky init
```
...
|------|---------|---------|
| Add E2E framework | `bun i -D playwright @playwright/test` | Test user flows |
...
## Required bun Scripts (Adapt for Your Language)
...
```json
{
  "scripts": {
    ...
    "precommit": "bun run typecheck && bun run lint && bun run check:file-length && bun run test"
  }
}
```
